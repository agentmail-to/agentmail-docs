...
Use an AgentMail inbox address that is not already used by a Google Workspace user, alias, or group. With this routing rule, Google handles recognized addresses, so creating the same address in AgentMail will not cause messages to be forwarded there.
