...
[Authorize Inbox](/api-reference/inboxes/authorize) and `auth_token`,
as this guide shows. If the agent is choosing the browser itself, call
[Connect Provider](/api-reference/providers/connect) and open the
...
async function createBrowserApiKey({
  ...
  const response = await fetch(AUTHORIZE_ENDPOINT.replace("{inbox_id}", encodeURIComponent(inboxId)), {
    ...
    body: JSON.stringify({ auth_token: action.auth_token }),
    redirect: "error",
  });
...
| Operation | Request |
| --- | --- |
| Attach a browser already on an AgentID page | `POST /v0/inboxes/{inbox_id}/authorize` with `auth_token` |
| Enroll a browser by magic URL | `POST /v0/providers/{provider_id}/connect` |
...
Approval is now the ordinary authenticated call described above, and the
persistent credential is the inbox-scoped public key the browser creates for
itself. A key registered for the old flow still lists as `type: public_key`
and can be deleted with `DELETE /v0/api-keys/{api_key_id}`, but it no longer
approves anything.
