...
one. There is no endpoint that extends a key in place.
...
`provider_owner_share`, snapshotted from that bearer key at creation and
...
`provider_owner_share` is one grouped grant covering both owner scopes. It is
snapshotted as `true` only when the creating bearer key holds
`provider_owner_share`, or both of the older `owner_profile` and `owner_email`
grants; a bearer key holding just one of those mints keys with
`provider_owner_share: false`, and can no longer pre-approve that single
scope.
