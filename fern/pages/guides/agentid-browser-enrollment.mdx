...
description: Verify the AgentID browser origin, attach a browser to a sign-in with ordinary authenticated HTTP, and manage the public keys browsers hold.
---
...
[AgentID](https://www.agentid.com) relying parties. It is an API key of type `browser`: it has no secret,
...
5. Send `{transaction_jti}` with the bearer API key to exactly
   ...
   response is the pending public key, read back at
...
6. Keep the existing AgentID page open. It observes the pending key, creates
   a non-extractable key, proves possession, and continues the same
   transaction.
...
| List an inbox's public keys, including those browsers hold | `GET /v0/api-keys?type=public_key&inbox_id={inbox_id}` |
...
For rotation, connect and verify a new key, then delete the old
one. There is no endpoint that extends a key in place. A key a browser holds
...
Inbox-scoped public keys are the current path for persistent browser approval. The
