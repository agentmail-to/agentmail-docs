...
The inbox's `status` is `inactive`, which blocks sending (including scheduled sends), creating or updating drafts, and inbound delivery. Reading, listing, and deleting still work. This is a state you control, not an account-level block: reactivate the inbox with `PATCH /v0/inboxes/{inbox_id}` and `{ "status": "active" }`, then retry.
