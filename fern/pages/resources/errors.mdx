...
Requests with inline Base64 attachments are limited to 6 MB total, including the message body, metadata, and encoded attachment content.

Reduce the attachment size or provide larger attachments through the `url` field. Keep URL-backed raw attachment data around 30 MB total per message. The complete encoded email, including attachments, body, and headers, must remain under 40 MB.
