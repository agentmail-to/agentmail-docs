...
Requests with inline Base64 attachments are limited to 6 MB total, including the message body, metadata, and encoded attachment content. Because Base64 encoding increases file size by about one third, 4.5 MB of raw files can reach the request limit before the rest of the message is counted.

Reduce the attachment size or provide each larger attachment through its `url` field. URL-backed attachments support approximately 30 MB total per message, and each URL must return 200 without authentication.
