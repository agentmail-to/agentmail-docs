...
No `Authorization` header was sent, or the header did not use the required `Bearer ` scheme (the message distinguishes the two). Send your API key as `Authorization: Bearer <api_key>`. Create a key at [console.agentmail.to](https://console.agentmail.to) or via `POST /v0/api-keys`.
