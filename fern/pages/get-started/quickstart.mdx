...
Your agent can sign up for its own inbox directly, no dashboard or console needed.

**1. Sign up:** the agent registers itself using the email you provide and gets an API key back.

Just provide your email address (or another one that is owned by you, the human). This flow is for first-time users only. Human email addresses already signed up with AgentMail will not work here. We'll send an OTP to that email, and once verified, the agent is mapped to that human and ready to send and receive.
