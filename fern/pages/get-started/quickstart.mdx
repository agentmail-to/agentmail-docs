...
Pick the path that matches how you're getting started:

- [**Agent start**](#agent-start): your agent signs itself up programmatically, no Console needed.
- [**Human start**](#human-start): you're a developer setting up AgentMail from the Console.

Both paths end with a working inbox that can send and receive email.
...
<Steps>
  ...
  <Step title="Create an inbox and send an email">
    ...
    <CodeBlocks>
      ...
      ```typescript title="TypeScript"
      // create an inbox (clientId enables safe retries)
      const inbox = await authedClient.inboxes.create({
