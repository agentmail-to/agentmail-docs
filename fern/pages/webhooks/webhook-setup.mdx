...
- **Hydrate messages via the API**: Webhook payloads are capped at 1 MB. When this limit is exceeded, the `html` body is omitted from the payload. Use the [Get Message](/api-reference/messages/get) endpoint to fetch the complete message content, including the full HTML body and attachments. See [Payload size limit](/overview#payload-size-limit) for details
