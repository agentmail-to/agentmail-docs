...
Instead of building a reply or forward by hand, you can create a `Draft` directly from an existing `Message`. Pass `in_reply_to` or `forward_of` to `inboxes.drafts.create` and AgentMail derives the recipients, subject, and threading from the source message, so a human can review the prepared reply before it goes out. This creates a `Draft` (it does not send) — send it later with `inboxes.drafts.send`.
...
To reschedule, update `send_at` with a new time. To keep the draft but cancel the scheduled send, update `send_at` to `null` — this clears the schedule and the `scheduled` label, leaving a plain draft you can edit or send manually. To discard the draft entirely, delete it.
