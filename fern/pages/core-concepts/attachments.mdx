...
<CodeBlocks>
   ...
   *
   * - messages.getAttachment(inboxId, messageId, attachmentId) → Buffer/Blob
   * - threads.getAttachment(inboxId, threadId, attachmentId) → Buffer/Blob
   */
  ...
  const data = await client.inboxes.messages.getAttachment("inbox@am.to", "msg_123", "att_456");
  ```
</CodeBlocks>
