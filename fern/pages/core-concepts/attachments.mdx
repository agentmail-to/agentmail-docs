...
- **`url`**: A publicly accessible URL that AgentMail can use to download the file. The URL must return a `200` response without authentication.
...
| Remote `url` | Approximately 30 MB of attachments per message | The combined size of the files AgentMail downloads from all attachment URLs. |
