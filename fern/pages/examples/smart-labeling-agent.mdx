...
- Priority-based inbox organization
- Multi-dimensional email analytics
- Learning advanced AgentMail features
...
if __name__ == '__main__':
    print("🏷️  SMART EMAIL LABELING AGENT\n")
...
|----------|----------------|
| `AGENTMAIL_API_KEY` | [AgentMail Dashboard](https://docs.agentmail.to/quickstart) |
| `OPENAI_API_KEY` | [OpenAI Platform](https://platform.openai.com/api-keys) |
| `NGROK_AUTHTOKEN` | [Ngrok Dashboard](https://dashboard.ngrok.com/get-started/your-authtoken) |
| `WEBHOOK_DOMAIN` | [Ngrok Domains](https://dashboard.ngrok.com/cloud-edge/domains) (claim a free domain) |
...
**Why this classification?**
- **Sentiment**: neutral (factual, no emotion)
- **Category**: bug-report (reporting technical error)
- **Priority**: high (blocking work, but not emergency)
- **Department**: technical (API issue)
...
**Filter by priority:**
- Search `urgent` to find emails needing immediate attention
- Search `high` to prioritize important issues
- Search `normal` and `low` to batch process during downtime

**Filter by department:**
- Search `sales` to route to your sales team
- Search `support` to handle customer issues
- Search `billing` to process payment inquiries
- Search `technical` to send to engineering
...
**Step 1: Update the valid values** in `analyze_email()`:
...
valid_values = {
    ...
    "priority": {"urgent", "high", "normal", "low"},
    "department": {"sales", "support", "billing", "technical"},
    "language": {"english", "spanish", "french", "german", "other"}  # New!
}
```
...
Return JSON with keys: sentiment, category, priority, department, language
"""
```

**Step 3: Add the label** in `apply_labels()`:
...
labels = [
    ...
    f"{classifications['priority']}",
    f"{classifications['department']}",
    f"{classifications['language']}"  # New!
]
```

### Changing Classification Values

Want different priority levels? Update the AI prompt:
...
- p1: Major issue (4 hour SLA)
- p2: Minor issue (24 hour SLA)
- p3: Enhancement (1 week SLA)
"""
```

Don't forget to update `valid_values`:
...
### Building Automation on Top of Labels

Once emails are labeled, you can build powerful automation:
