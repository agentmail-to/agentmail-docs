...
AgentMail sets the policy to `reject` by default, which means any email that fails SPF or DKIM checks will be rejected outright. The `rua` tag sends aggregate reports to AgentMail so we can monitor your domain's deliverability health.
