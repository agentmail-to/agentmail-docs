...
Make sure you are adding the exact record type AgentMail provides. AgentMail uses TXT records for SPF, DMARC, and new DKIM selector verification, plus MX records when the domain receives mail. Older domains may still use DKIM CNAME records.

| Record | Correct type |
| --- | --- |
| SPF | TXT |
| DKIM | TXT for domains created on or after July 6, 2026; CNAME for legacy domains if shown |
| DMARC | TXT |
| MX | MX |
...
<Note>
As of July 6, 2026, new DKIM records are authenticated with custom TXT selector records instead of CNAME records. For existing domains, do not convert a working CNAME unless AgentMail provides replacement TXT records.
</Note>
...
**What to do:** Check your DNS dashboard for conflicts on the DKIM selector host. Keep the current DKIM record from AgentMail and remove stale or duplicate records that use the same host.
