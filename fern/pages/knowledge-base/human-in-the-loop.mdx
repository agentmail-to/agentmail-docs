...
Then build a dashboard or webhook listener that alerts a human when specific labels appear:
...
def handle_webhook():
    ...
    if payload["event_type"] == "message.received":
        ...
        # Check if agent flagged this for review
        if "needs-human-review" in message.get("labels", []):
...
Restrict which addresses your agent can email autonomously. For addresses outside the allowlist, route the message through a human approval flow using drafts.

```python
# Only allow autonomous sending to known customers
client.lists.create("send", "allow", entry="trusted-customer.com")

# For unknown recipients, create a draft instead of sending directly
def send_or_draft(inbox_id, to, subject, text, html):
    # Check if recipient is on the allowlist
    try:
        client.lists.get("send", "allow", entry=to)
        # Recipient is allowed, send directly
        client.inboxes.messages.send(
            inbox_id=inbox_id, to=[to],
            subject=subject, text=text, html=html
        )
    except:
        # Not on allowlist, create draft for human review
