...
async def connect_to_agent(email_address: str):
    ...
    client = AgentMail(base_url="https://api.agentmail.to/v0", api_key=os.getenv("AGENTMAIL_API_KEY"))
```
...
async def connect_to_agent(email_address: str):
    uri = f"ws://localhost:8000/ws/{email_address}"
    client = AgentMail(base_url="https://api.agentmail.to", api_key=os.getenv("AGENTMAIL_API_KEY"))
