...
- **Local validation is incomplete for query parameters.** Request bodies are type-checked locally, but query flags are not — an invalid value like `--limit abc` reaches the API and returns a 400 rather than being caught locally. `--dry-run` shows the request that would be sent, not a guarantee that the server will accept it.
