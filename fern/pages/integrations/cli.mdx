...
Version 1.5 adds `--allow-conflicting-provider true` to domain creation, including
`agentmail pods domains create`, so you can register a domain that already uses
Google Workspace. Configure DNS and inbound routing separately using the
[Google Workspace guide](https://docs.agentmail.to/google-workspace).

Provider responses now include optional `owner_signup_limit`: the maximum number
of accounts your organization can sign up at that provider. `0` pauses new
sign-ups; existing accounts can still sign in. Omission means no configured limit.

## Upgrading to 1.4

Public keys and AgentID sign-in keys use the same `api-keys` commands as bearer
keys. Upgrade with `npm install -g agentmail-cli@latest` to use these commands:

| Operation | Current command |
| --- | --- |
| Start a provider sign-in | `agentmail providers connect --provider-id <provider_id> --inbox-id <inbox_id>` |
| Authorize a pending sign-in | `agentmail inboxes authorize --inbox-id <inbox_id> --auth-token <auth_token>` |
| Accept a provider's disclosure | Add `--accept-disclosure true` to connect or authorize |
| List public keys | `agentmail api-keys list --type public_key` |
| Register a public key | `agentmail api-keys create --public-key '<jwk>'` |
| Check sign-in key status | `agentmail api-keys get --api-key-id <api_key_id>` |
| Rename a key | `agentmail api-keys update --api-key-id <api_key_id> --name <name>` |
| Revoke a key or cancel a pending sign-in key | `agentmail api-keys delete --api-key-id <api_key_id>` |

Connect and authorize require `provider_connect`. Accepting a disclosure that
shares the owner's name or email also requires `provider_share_owner`. Connect
...
[AgentID Sign-In](https://docs.agentmail.to/agentid-sign-in) for the full flow.
