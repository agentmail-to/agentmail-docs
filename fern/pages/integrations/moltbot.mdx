...
There are two ways to integrate AgentMail with Moltbot: using the AgentMail MCP server or creating a custom skill.

## Option 1: MCP Server (Recommended)

The simplest way to add email capabilities to Moltbot is through the AgentMail MCP server. Moltbot supports MCP (Model Context Protocol) servers, which allows it to connect to external tools like AgentMail.
