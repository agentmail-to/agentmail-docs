...
**Create Draft now builds replies and forwards.** Pass a source message to `inboxes.drafts.create` and AgentMail derives the recipients, subject, and threading for you:
- `in_reply_to` — create a draft replying to the sender. Add `reply_all` to address the whole thread.
- `forward_of` — create a draft forwarding the message.
